...
const ProfilePage = () => {
  ...
  const handleConfirmReschedule = () => {
    ...
    dispatch(rescheduleBooking({ bookingId: rescheduleModal.booking._id, newEventDateId: selectedNewDateId }))
      ...
      .catch((err) => alert(typeof err === 'string' ? err : 'Reschedule failed'));
  };
                  ...
                  {myBookings.map((b) => (
                    <div key={b._id} className="flex items-center justify-between p-4">
                      <div>
                        ...
                        {(() => {
                          const bookedDate = b.eventDateId && b.event?.eventDates?.length
                            ? b.event.eventDates.find((d) => d._id === b.eventDateId)
                            : null;
                          const displayDate = bookedDate?.date || b.event?.date;
                          return (
                            <p className="text-xs text-gray-500 mt-0.5">
                              {displayDate ? new Date(displayDate).toLocaleDateString('en-IN') : ''}
                              {bookedDate?.label ? ` · ${bookedDate.label}` : ''}
                              {b.event?.time ? ` · ${b.event.time}` : ''} · {b.quantity} ticket(s)
                            </p>
                          );
                        })()}
                        {b.event?.venue?.name && (
                          <p className="text-xs text-gray-400 mt-0.5 flex items-center gap-1">
                            <MapPin size={10} /> {b.event.venue.name}
                            {b.event.venue.address ? `, ${b.event.venue.address}` : ''}
                          </p>
                        )}
                        {(() => {
                          const organizerDisplayName = b.event?.organizerName?.trim() || b.event?.organizer?.displayName;
                          return organizerDisplayName ? (
                            <p className="text-xs text-gray-400 mt-0.5">Organizer: {organizerDisplayName}</p>
                          ) : null;
                        })()}
          ...
          <div className="bg-white w-full max-w-sm p-6 border border-gray-200">
            ...
            <div className="space-y-2 mb-5">
              {rescheduleModal.dates.map((d) => (
                <label
                  key={d._id}
                  className={`flex items-center gap-3 border px-4 py-3 cursor-pointer transition-colors ${selectedNewDateId === d._id ? 'border-brand-red bg-red-50' : 'border-gray-200'
                    }`}
                >
                  <input
                    type="radio"
