...
export const fetchTonight = createAsyncThunk(
  "events/fetchTonight",
  async (_, { rejectWithValue }) => {
    try {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchEvents.fulfilled, (s, a) => {
        ...
        s.pagination.hasMore =
          (a.payload.page ?? page) < (a.payload.totalPages ?? 1);
      })
    ...
    builder
      ...
      .addCase(fetchEventById.rejected, (s, a) => {
        s.detailStatus = "failed";
        s.error = a.payload;
      });

    // fetchLiveNow
    builder.addCase(fetchLiveNow.fulfilled, (s, a) => {
      s.liveNow = a.payload;
    });

    // fetchTonight
    builder
      .addCase(fetchTonight.pending, (s) => {
        s.tonightStatus = "loading";
      })
      .addCase(fetchTonight.fulfilled, (s, a) => {
        s.tonightStatus = "succeeded";
        s.tonight = a.payload;
      })
      .addCase(fetchTonight.rejected, (s, a) => {
        s.tonightStatus = "failed";
        s.error = a.payload;
      });
    ...
    // fetchPendingEvents
    builder
      .addCase(fetchPendingEvents.pending, (s) => {
        s.pendingStatus = "loading";
      })
      .addCase(fetchPendingEvents.fulfilled, (s, a) => {
        s.pendingStatus = "succeeded";
        s.pending = a.payload;
      })
      .addCase(fetchPendingEvents.rejected, (s, a) => {
        s.pendingStatus = "failed";
        s.error = a.payload;
      });

    // approveEvent
    builder.addCase(approveEvent.fulfilled, (s, a) => {
      s.pending = s.pending.filter((e) => e.id !== a.payload);
    });

    // rejectEvent
    builder.addCase(rejectEvent.fulfilled, (s, a) => {
      s.pending = s.pending.filter((e) => e.id !== a.payload);
    });
  },
});
