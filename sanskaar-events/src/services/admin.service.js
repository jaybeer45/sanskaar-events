...
export const adminService = {
  ...
  rejectEvent: (id) => axiosInstance.patch(`/admin/events/${id}/reject`),
  ...
  verifyOrganizer: (id) => axiosInstance.patch(`/admin/organizers/${id}/verify`),
  rejectOrganizer: (id) => axiosInstance.patch(`/admin/organizers/${id}/reject`),
  ...
  getVendorPayouts: async (status) => {
    const res = await axiosInstance.get('/admin/vendor-payouts', { params: status ? { status } : {} });
    return normalizeList(res.data.results);
  },
  markPayoutPaid: (id, bankRef) => axiosInstance.patch(`/admin/vendor-payouts/${id}/mark-paid`, { bankRef }),
};