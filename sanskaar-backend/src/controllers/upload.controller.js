...
// @route POST /api/v1/uploads/event-video
// @desc  Public — returns URL for the event form's promotional video field
const uploadEventVideo = asyncHandler(async (req, res) => {
  if (!req.file) {
    res.status(400);
    throw new Error("A video file is required.");
  }

  const url = await storage.saveEventVideo(req.file, req);
  res.status(200).json({ url, mimeType: req.file.mimetype });
});
