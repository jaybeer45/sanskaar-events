...
const getPendingEvents = asyncHandler(async (req, res) => {
  const results = await Event.find({ status: "pending_approval" }).populate(
    "organizer",
    "name email",
  );
...
const rejectEvent = asyncHandler(async (req, res) => {
  ...
  if (!reason || reason.trim().length < 5) {
    res.status(400);
    throw new Error(
      "A valid reason (min 5 characters) is required to reject an event.",
    );
  }
...
const getPendingVendors = asyncHandler(async (req, res) => {
  const results = await Vendor.find({ isApproved: false }).populate(
    "user",
    "name email",
  );
  res.status(200).json({ results });
...
const getPendingOrganizers = asyncHandler(async (req, res) => {
  const results = await Organizer.find({ kycStatus: "submitted" }).populate(
    "owner",
    "name email",
  );
...
const verifyOrganizer = asyncHandler(async (req, res) => {
  ...
  await User.findByIdAndUpdate(organizer.owner, {
    $addToSet: { roles: "organizer" },
  });

  res
    .status(200)
    .json({ success: true, id: organizer._id, kycStatus: organizer.kycStatus });
});
...
const createManualCoupon = asyncHandler(async (req, res) => {
  const { code, valuePaise, maxUses, expiresAt, userEmail, targetRole } =
    req.body;
  ...
  if (targetRole && !VALID_TARGET_ROLES.includes(targetRole)) {
    res.status(400);
    throw new Error(
      `targetRole in me se ek hona chahiye: ${VALID_TARGET_ROLES.join(", ")}`,
    );
  }
...
const getManualCoupons = asyncHandler(async (req, res) => {
  const coupons = await Coupon.find({ type: "admin_manual" }).sort({
    createdAt: -1,
  });
...
const getAllVendorBookings = asyncHandler(async (req, res) => {
  const results = await VendorBooking.find({})
...
// @route GET /api/v1/admin/events/history
const getEventsHistory = asyncHandler(async (req, res) => {
...
// @route GET /api/v1/admin/organizers/history
const getOrganizersHistory = asyncHandler(async (req, res) => {
...
// @route GET /api/v1/admin/vendors/history
const getVendorsHistory = asyncHandler(async (req, res) => {
  const results = await Vendor.find({ isApproved: true })
    .populate("user", "name email")
    .sort({ updatedAt: -1 })
    .limit(50);
  res.status(200).json({ results });
