...
// Public — event promotional video. Single file, returns one URL.
const saveEventVideo = async (file, req) => {
  const dir = path.join(uploadsRoot, "events", "videos");
  ensureDir(dir);

  const filename = makeFilename(file.originalname);
  fs.writeFileSync(path.join(dir, filename), file.buffer);
  return `${req.protocol}://${req.get("host")}/uploads/events/videos/${filename}`;
};
